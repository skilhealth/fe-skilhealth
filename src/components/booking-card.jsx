...
function BookingCard({ data }) {
    ...
    return (
        <div className={`${data.status?"bg-slate-500":"bg-slate-200"} w-full h-36 lg:h-40 flex p-4 gap-3 rounded-xl`} onClick={toDetailPage}>
            <div className="h-full">
                <img src={`${data.Dokter.images}`} alt="" className="h-full rounded-xl object-cover" />
            </div>
            <div className="flex flex-col">
                <div className="text-sm flex items-center gap-2">
                    <div className={` p-1 rounded-lg ${tipe(data)}`}>{data.Jadwal.tipe}</div>
